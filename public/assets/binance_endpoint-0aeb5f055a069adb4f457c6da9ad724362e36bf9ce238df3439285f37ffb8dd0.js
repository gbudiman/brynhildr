...
var BinanceEndpoint = function() {
	...
	var aggregate_data = function(crypto, exchange, data) {
		...
		$.each(aggregate_buffer[crypto], function(xchg, data) {
			r.push([exchange, data.low / forex_value, data.high / forex_value])
			c.push([exchange, data.close / forex_value])
		})
			...
			if (aggregate_init[crypto] == undefined) {
				$('.' + chart_class).each(function() {
					...
					Highcharts.chart( {
						...
						xAxis: {
							categories: ['bithumb', 'coinone', 'korbit'],
							tickColor: '#eee',
							labels: {
								style: {
									color: '#eee'
