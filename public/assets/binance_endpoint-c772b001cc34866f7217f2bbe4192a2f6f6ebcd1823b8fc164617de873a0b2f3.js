...
var BinanceEndpoint = function() {
	...
	var resize = function() {
		setTimeout(function() {
			...
			$.each(aggregate_init, function(key, _junk) {
				var chart_class = $('.aggregate-' + key)
				$.each(chart_class, function() {
					console.log($(this))
					var chart = $(this).highcharts()
					chart.setSize(chart_width * 2, chart_height * 3)
				})
			})
		}, 250)
	...
	var update_local_ticker = function(pair) {
		...
		$.each(l_pairs, function(exchange, data) {
			...
			$.each(['open', 'high', 'low', 'close'], function(_junk, x) {
				...
				if (!isNaN(value)) {
					...
					var dom_nom = '#local-nominal-' + pair + '-' + x + '-' + exchange
					var dom_pctg = '#local-pctg-' + pair + '-' + x + '-' + exchange
					var pctg_val = (value - equitether_buffer[pair]) / equitether_buffer[pair]

					$(dom_nom).text('$' + value.toFixed(2))
					$(dom_pctg).text((pctg_val * 100).toFixed(0) + '%')

					if (pctg_val > 0) {
						$(dom_pctg).removeClass('negative')
					} else {
						$(dom_pctg).addClass('negative')
					}
				}
			})
		})
		...
		render_aggregate_chart(l_pairs)
		
	}

	var aggregate_data = function(crypto, exchange, data) {
		aggregate_buffer[crypto][exchange] = data
		var b = aggregate_buffer[crypto][exchange]
		var r = new Array()
		var c = new Array()

		$.each(['bithumb', 'coinone', 'korbit', 'gdax'], function(_junk, xchg) {

			var xdata = aggregate_buffer[crypto][xchg]
		//$.each(aggregate_buffer[crypto], function(xchg, xdata) {
			if (xdata == undefined) {
				return true
			}
			var forex_value = local_ticker['currency_layer'][xdata.quote_currency]
			r.push([xchg, xdata.low / forex_value, xdata.high / forex_value])
			c.push([xchg, xdata.close / forex_value])
		})
	...
	var zero_cross = function(dom_object, crypto) {
		var eqb = equitether_buffer[dom_object.attr('data-pair')]
		var z = new Array()
		$.each(['bithumb', 'coinone', 'korbit', 'gdax'], function(_junk, xchg) {
			if (aggregate_buffer[crypto][xchg] == undefined) return true
			z.push([xchg, eqb])
		})
	...
	var margin_formatter = function(dom_object, val) {
		var eqb = equitether_buffer[dom_object.attr('data-pair')]
		var pctg = (val - eqb) / eqb

		if (isNaN(pctg)) return '--%'
		return (pctg * 100).toFixed(2) + '%'
	}

	var render_aggregate_chart = function(w) {
		//console.log(w)
		var exchange = w.exchange
		if (exchange == 'currency_layer') return


		$.each(w.data, function(crypto, data) {
			var chart_class = 'aggregate-' + crypto
			if (aggregate_buffer[crypto] == undefined) {
				aggregate_buffer[crypto] = {}
			}

			if (aggregate_init[crypto] == undefined) {
				$('.' + chart_class).each(function() {
					var ab_buffer = aggregate_data(crypto, exchange, data)
					var zc = zero_cross($(this), crypto)
					var el_class = $(this)[0]
					var that = $(this)
					Highcharts.chart( {
						chart: {
							...
							width: get_proper_chart_width() * 2
						},
						credits: false,
						legend: { enabled: false },
						navigator: { enabled: false },
						...
						series: [{
							...
							dataLabels: {
								enabled: true,
								formatter: function() {
									return margin_formatter(that, this.y)
								},
								style: {
									color: '#eee'
								}
						...
						xAxis: {
							categories: ['bithumb', 'coinone', 'korbit', 'gdax'],
							tickColor: '#eee',
							labels: {
								style: {
									color: '#eee'
				...
				$('.' + chart_class).each(function() {
					...
					chart.series[0].setData(ab_buffer.area)
					chart.series[1].setData(ab_buffer.close)
					chart.series[2].setData(zc)
				})
			}
	...
	var append_dom = function() {
		...
		$.each(pairs, function(_junk, pair) {
			...
			s +=     '</div>'
			s +=   '</div>'
			s +=   '<div class="colfig col-xs-12 aggregate-' + pair.slice(0, 3).trim() + '" '
				+      'data-pair="' + pair + '">'
				+    '</div>'

			// s +=   '<div class="col-xs-2 colfig" />'
			//   +      '<div class="col-xs-3 colfig text-center">Bithumb</div>'
			//   +      '<div class="col-xs-3 colfig text-center">Coinone</div>'
			//   +      '<div class="col-xs-3 colfig text-center">Korbit</div>'
			  

			// $.each(['open', 'high', 'low', 'close'], function(_junk, prop) {
			// 	s +=   '<div class="col-xs-2 colfig">' + prop[0].toUpperCase() + '</div>'

			// 	$.each(['bithumb', 'coinone', 'korbit'], function(_junk, xchg) {
			// 		s += '<div class="col-xs-2 colfig">'
			// 			+    '<span class="local-ticker pull-right" id="local-nominal-' + pair + '-' + prop + '-' + xchg + '">--</span>'
			// 		  +  '</div><div class="col-xs-1 colfig">'
			// 		  +    '<span class="local-ticker pull-right" id="local-pctg-' + pair + '-' + prop + '-' + xchg + '">--</span>'
			// 		  +  '</div>'
			// 	})
			// })
			s += '</div>'
